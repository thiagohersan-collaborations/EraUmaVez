...
class WaveformRenderer implements AudioListener {
  ...
  synchronized PVector getPoints() {
    return new PVector(float(cnt), maxL, maxR);
  }
  ...
  synchronized void draw() {
    ...
    if ( left != null && right != null ) {
      ...
      maxL = (left[0] + left[left.length/4] + left[left.length/2] + left[left.length-1])/1.0;
      ...
      maxR = (right[0] + right[right.length/4] + right[right.length/2] + right[right.length-1])/1.0;
    }
    else if ( left != null ) {
      noFill();
      stroke(255);
